...
export const HeroAnimationSection: React.FC<HeroAnimationSectionProps> = ({
  ...
  return (
    <section 
      ref={sectionRef}
      className={`relative min-h-screen flex items-center justify-center overflow-hidden ${sizeClasses[size]} bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900`}
    >
      {/* Animated Background */}
      ...
      {showFloatingElements && (
        <div className="absolute inset-0 pointer-events-none">
          {floatingElements.map((element, index) => (
            <div
              key={index}
              className={`absolute ${element.position} text-blue-400/60 animate-bounce`}
              style={{ animationDelay: `${element.delay}ms`, animationDuration: '3s' }}
            >
              {element.icon}
            </div>
          ))}
      ...
      {showWave && (
        <div className="absolute bottom-0 left-0 right-0 h-32 overflow-hidden">
          <div className="absolute bottom-0 left-0 right-0 h-full bg-gradient-to-t from-blue-600/20 to-transparent">
            <svg
            ...
            >
              <path
                ...
                opacity=".25"
                className="fill-current text-blue-400/30"
              />
              <path
                d="M0,0V15.81C13,36.92,27.64,56.86,47.69,72.05,99.41,111.27,165,111,224.58,91.58c31.15-10.15,60.09-26.07,89.67-39.8,40.92-19,84.73-46,130.83-49.67,36.26-2.85,70.9,9.42,98.6,31.56,31.77,25.39,62.32,62,103.63,73,40.44,10.79,81.35-6.69,119.13-24.28s75.16-39,116.92-43.05c59.73-5.85,113.28,22.88,168.9,38.84,30.2,8.66,59,6.17,87.09-7.5,22.43-10.89,48-26.93,60.65-49.24V0Z"
                opacity=".5"
                className="fill-current text-blue-400/20"
              />
              <path
                d="M0,0V5.63C149.93,59,314.09,71.32,475.83,42.57c43-7.64,84.23-20.12,127.61-26.46,59-8.63,112.48,12.24,165.56,35.4C827.93,77.22,886,95.24,951.2,90c86.53-7,172.46-45.71,248.8-84.81V0Z"
                className="fill-current text-blue-400/10"
              />
            </svg>
          </div>
        </div>
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <div className="max-w-4xl mx-auto">
          ...
          <Badge 
            variant="secondary" 
            className="mb-6 bg-white/10 text-white border-white/20 hover:bg-white/20 backdrop-blur-sm animate-pulse"
          >
          ...
          {/* Subtitle */}
          {subtitle && (
            <p className="text-xl sm:text-2xl text-blue-100 mb-4 font-medium animate-fade-in-up" style={{ animationDelay: '0.8s' }}>
              {subtitle}
            </p>
          )}

          {/* Description */}
          <p className="text-lg sm:text-xl text-blue-50 mb-8 max-w-2xl mx-auto leading-relaxed animate-fade-in-up" style={{ animationDelay: '1s' }}>
            {description}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-12 animate-fade-in-up" style={{ animationDelay: '1.2s' }}>
            ...
            {secondaryCTA && (
              <Button
                variant="outline"
                size="lg"
                className="border-white/30 text-white hover:bg-white/10 backdrop-blur-sm transform hover:scale-105"
